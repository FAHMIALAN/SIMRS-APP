...
<div class="card p-8">
    <div class="mb-8 border-b border-slate-100 pb-6">
        <h2 class="text-2xl font-bold text-slate-800">Buat Peresepan Baru</h2>
        <p class="text-slate-500 mt-1">Input data pasien, dokter, dan daftar obat yang diresepkan.</p>
    </div>
    
    <form action="<?= base_url('admin/peresepan/store') ?>" method="POST" id="form-peresepan" class="space-y-8">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="relative group">
                    ...
                    <select name="pasien_id" class="input-field pl-11 bg-slate-50 appearance-none" required>
                        ...
                        <?php foreach($pasien as $p): ?>
                            <option value="<?= $p['id_pasien'] ?>"><?= $p['nama'] ?> (<?= $p['nomor_rm'] ?>)</option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            ...
            <div class="md:col-span-1">
                ...
                <div class="relative group">
                    ...
                    <input type="date" name="tanggal" class="input-field pl-11" value="<?= date('Y-m-d') ?>" required>
                </div>
            </div>
        </div>
        ...
        <div class="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                ...
                <table class="w-full text-left">
                    ...
                    <tbody id="obat-container" class="divide-y divide-slate-100">
                        <tr class="obat-row">
                            ...
                            <td class="py-4 px-4">
                                <input type="number" name="jumlah[]" class="input-field jumlah-input bg-white py-2 text-center" value="1" min="1" required>
                            </td>
                            <td class="py-4 px-4 text-right">
                                <span class="subtotal-text text-sm font-bold text-slate-800">Rp 0</span>
                            </td>
                            <td class="py-4 px-4 text-center">
                                <button type="button" class="w-9 h-9 flex items-center justify-center bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white rounded-xl transition-all btn-hapus" disabled>
            ...
            <div class="mt-6 flex flex-col sm:flex-row justify-between items-center gap-6 pt-6 border-t border-slate-200">
                ...
                <div class="flex flex-col items-end">
                    ...
                    <span id="total-semua" class="text-3xl font-black text-blue-600 tracking-tight">Rp 0</span>
                </div>
            </div>
        </div>
        ...
        <div class="pt-8 flex items-center justify-end gap-4">
            ...
            <button type="submit" class="px-10 py-3 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-2xl shadow-xl shadow-blue-200 transition-all active:scale-[0.98]">
                <i class="fas fa-file-check mr-2"></i> Konfirmasi & Simpan Resep
            </button>
        </div>
    </form>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function attachEvents(row) {
        ...
        select.addEventListener('change', function() {
            const maxStok = this.options[this.selectedIndex].getAttribute('data-stok');
            if(maxStok > 0) {
                jumlah.max = maxStok;
                if(parseInt(jumlah.value) > parseInt(maxStok)) {
                    jumlah.value = maxStok;
                }
            }
            hitungTotal();
        });
    ...
    btnTambah.addEventListener('click', function() {
        ...
        row.querySelector('.obat-select').selectedIndex = 0;
        row.querySelector('.jumlah-input').value = 1;
        row.querySelector('.harga-text').textContent = 'Rp 0';
        row.querySelector('.subtotal-text').textContent = 'Rp 0';
        row.querySelector('.jumlah-input').removeAttribute('max');
    ...
    attachEvents(document.querySelector('.obat-row'));
    updateHapusButtons();
});
</script>
